...
check "PERF-5 : le client A lit toujours uniquement son dossier" "1|0" "$(q02_client "select count(*) from public.v_mes_devis; select count(*) from public.devis_envois;" | tail -2 | paste -sd'|')"
