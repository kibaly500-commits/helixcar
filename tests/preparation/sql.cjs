...
(async()=>{
 ...
 const remiseMigration=fs.readdirSync('supabase/migrations').find(x=>x.endsWith('_point_remise_sans_enseigne.sql'));
 if(remiseMigration)await db.exec(fs.readFileSync('supabase/migrations/'+remiseMigration,'utf8'));
 ...
 if(remiseMigration)check('point privé sans enseigne',src.point_remise==='12 rue de l’Université, 93160 Noisy-le-Grand');
 ...
 create or replace view public.v_mes_missions as select m.id,m.prix_ttc from missions m join clients c on c.id=m.client_id where c.auth_user_id=auth.uid();`);
 await db.exec(fs.readFileSync('supabase/migrations/20260926174317_preparation_complete_confidentialite_client.sql','utf8'));
 await db.exec(`delete from test_infos;insert into test_infos values('${cid}','attendue')`);
 await rejected('Préparation refusée avant complétude',()=>rpc('source_preparation_missions',[cid]));
 await db.exec(`update test_infos set statut='transmise'`);
 await rejected('Préparation refusée avant validation admin',()=>rpc('source_preparation_missions',[cid]));
 await db.exec(`update test_infos set statut='validee'`);
 check('Préparation accessible après validation',!!(await rpc('source_preparation_missions',[cid])).client);
 await db.exec(`update clients set auth_user_id='${uid}' where id='${cid}';update missions set prix_ttc=9876 where client_id='${cid}';select set_config('request.jwt.claim.sub','${uid}',false)`);
 const clientRows=(await db.query('select * from v_mes_missions')).rows;
 check('Montant interne absent de la réponse client',clientRows.length>0&&clientRows.every(m=>m.prix_ttc===null));
 await db.exec(`select set_config('request.jwt.claim.sub','66666666-6666-6666-6666-666666666666',false)`);
 check('Vue toujours limitée au propriétaire',(await db.query('select * from v_mes_missions')).rows.length===0);
