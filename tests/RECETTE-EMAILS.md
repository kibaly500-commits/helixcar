...
| **EM0-001b** Confirmation d'adresse (client) | Titulaire de l'adresse | `_sb.auth.signUp({email, password, options: {emailRedirectTo: HELIXCAR_URL_DASHBOARD}})` : le retour vise `/dashboard.html` sur l'origine réellement servie, jamais un paramètre utilisateur ni le « Site URL » générique | Supabase Auth « Confirm signup » | `supabase/templates/confirmation-adresse.html` (objet à saisir : « Confirmez votre adresse e-mail HelixCar », variable `{{ .ConfirmationURL }}`) — **déploiement dans Supabase NON VÉRIFIÉ** | adresse, lien de confirmation à usage unique fourni par Supabase | hors dépôt | Code couvert par `tests/t_rattachement.js`, y compris confirmation requise ; réception réelle BLOQUÉE tant que l'origine utilisée n'est pas confirmée dans les Redirect URLs Supabase | Vérifier dans Supabase : confirmation activée, modèle collé, objet et URL de retour autorisée ; tester inscription sur PC puis confirmation sur téléphone |
