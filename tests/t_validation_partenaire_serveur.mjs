...

{
  ...
  check('V3 : le futur domaine officiel configuré garde la priorité et le lien porte une preuve signée',
...
{
  const d=double();
  const ancien={...env,HELIXCAR_URL_PUBLIQUE:'https://helixcar-i89b.vercel.app'};
  await traiterRequete(d.sb,req({action:'valider',convoyeur_id:ID}),ancien,d.fetch);
  check('V3b : l’ancien domaine i89b n’apparaît plus dans les nouveaux liens',
    !d.journal.envois[0].payload.text.includes('https://helixcar-i89b.vercel.app')
    && d.journal.envois[0].payload.text.includes('https://helixcar-i89b-git-codex-helix-b25bf8-kibaly500-commits-projects.vercel.app'));
}
{
