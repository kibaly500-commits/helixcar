...
await page.addInitScript(()=>{
 window.pdfjsLib={GlobalWorkerOptions:{},getDocument:()=>({promise:Promise.resolve({numPages:2,getPage:async()=>({getViewport:({scale})=>({width:595*scale,height:842*scale}),render:()=>({promise:Promise.resolve()})})})})};
...
if(width<=640){
 await page.waitForSelector('.pdf-rendu-mobile');
 assert.ok(await page.locator('.pdf-cadre').isVisible());
 assert.ok(await page.locator('.pdf-cadre iframe').isHidden());
 assert.equal(await page.locator('.pdf-mobile-page canvas').count(),2);
 assert.ok(await page.locator('.pdf-mobile-page canvas').evaluateAll(canvases=>canvases.every(c=>c.getBoundingClientRect().right<=innerWidth)));
}else{assert.ok(await page.locator('.pdf-cadre iframe').isVisible());}
