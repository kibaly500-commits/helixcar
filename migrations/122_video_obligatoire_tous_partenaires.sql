-- HelixCar — 122 : vidéo obligatoire pour toute candidature partenaire.
-- Une minute pour le convoyage seul ; deux minutes pour tout autre métier
-- ou combinaison. Migration additive : aucune candidature n'est supprimée.
begin;
...
begin
 ...
 v_max:=case
   when coalesce(cardinality(c.activites),0)=0 then 0
   when cardinality(c.activites)=1 and 'convoyage'=any(c.activites) then 60
   else 120 end;
