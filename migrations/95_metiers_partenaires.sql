...
-- La contrainte est retrouvée par sa DÉFINITION, jamais par un nom
-- supposé : PostgreSQL nomme automatiquement les CHECK déclarés en
-- ligne, et ce nom pourrait différer d'un projet à l'autre.
do $$
declare
  c record;
begin
  for c in
    select conname
     ...
     where conrelid = 'public.convoyeur_decisions'::regclass
       and contype  = 'c'
       and pg_get_constraintdef(oid) ilike '%activite%'
  loop
