...
(function (root) {
  ...
  function build(c,vehicles,pointAdresse) {
    ...
    vs.forEach(raw=>{
      ...
      const base={type_mission:'convoyage',marque_modele:v.marque_modele,type_vehicule:v.type_vehicule,immatriculation:v.immatriculation,vin:v.vin,nb_vehicules:1,
        ...
        restitution:!!v.restitution_concernee,adresse_restitution:[v.restit_adresse_rue,v.restit_code_postal,v.restit_ville].filter(Boolean).join(', '),restit_contact_nom:v.restit_contact_nom,restit_contact_tel:v.restit_contact_tel,restit_marque_modele:v.restit_marque_modele,restit_immatriculation:v.restit_immatriculation,restit_vin:v.restit_vin,restit_info:v.restit_contraintes,date_restitution_depart:stamp(v.restit_date,hour(v,'restit'))};
      const add=(leg)=>{
