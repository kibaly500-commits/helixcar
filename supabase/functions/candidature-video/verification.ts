...
export async function verifierObjet(sb:any,c:any,env:Record<string,string|undefined>,fetchFn:typeof fetch=fetch){
 ...
 if(configuree&&secret&&secret.length>=32){
   ...
   corpsWorker={url:sig.data.signedUrl,candidature_id:c.id,mime:c.video_envoi_mime,duree_max:c.activites.includes('renfort')||c.activites.includes('technicien')?120:60};
 }else{
